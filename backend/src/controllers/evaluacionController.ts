...
export class EvaluacionController {
  ...
  async listByExposicion(request: FastifyRequest, reply: FastifyReply) {
  ...
  async obtenerEvaluacionPropia(request: FastifyRequest, reply: FastifyReply) {
    const params = request.params as any;
    ...
    const alumnoId = await this.getAlumnoIdByUserId(currentUserId);
    if (!alumnoId) {
      return reply.status(404).send({ error: 'Perfil de alumno no encontrado' });
    }
    try {
      const evaluacion = await this.evaluacionService.obtenerEvaluacionPropia(exposicionId, alumnoId);
  ...
  private async getAlumnoIdByUserId(userId: string): Promise<number | null> {
    const alumno = await this.alumnoRepo.findByUsuarioId(userId);
